        ...
        for(i=0; i<p.qntdIO; i++){
            if(p.listaIO[i].t_chegada == 0){
                if(p.listaIO[i].tipo == 0){
                    ...
                    if(filaVazia(&disco)){
                       cont_reg_disco = T_DISCO; 
                    }
                    insereFila(&disco, p.PID);
                }
                else if(p.listaIO[i].tipo == 1){
                    printf("Processo %d pediu IO de fita\n", p.PID);
                    if(filaVazia(&fita)){
                       cont_reg_fita = T_FITA; 
                    }
                    insereFila(&fita, p.PID);
                    
                }
                else if(p.listaIO[i].tipo == 2){
                    printf("Processo %d pediu IO de impressora\n", p.PID);
                    if(filaVazia(&impressora)){
                       cont_reg_impressora = T_IMP;
                    }
                    insereFila(&impressora, p.PID);
                ...
                processos[p.PID].status = 2;
            }
        }
       }
        /* DECRESCIMENTO DE CADA IO */
    if((cont_reg_disco != -1) || (cont_reg_fita != -1) || (cont_reg_impressora != -1)){
       if (cont_reg_disco > 0) {
            cont_reg_disco--;
        } else if (cont_reg_disco == 0) {
            p_disco = peek(&disco);
            printf("Processo %d completou IO de disco e voltou para fila de baixa prioridade\n", p_disco);

            if((filaVazia(&alta)) && (filaVazia(&baixa))){ // se a fila alta e baixa estiverem vazia,
            // o processo alocado tem que poder fazer 2 ut de quantum
                count = 0;
            }

            insereFila(&baixa, p_disco); // Processo volta para a fila de baixa prioridade
            removeFila(&disco);

            if(filaVazia(&disco)){
                cont_reg_disco = -1; 
            }
            else if(!(filaVazia(&disco))){
                cont_reg_disco = T_DISCO;
            }
            
            processos[ p_disco].status = 0;
            processos[p_disco].qntdIO--;
        }

        if (cont_reg_fita > 0) {
            cont_reg_fita--;
        } else if (cont_reg_fita == 0) {
            p_fita = peek(&fita);
            printf("Processo %d completou IO de fita e voltou para fila de alta prioridade\n", p_fita);
            
            if(filaVazia(&alta)){ // se a fila alta estiver vazia, o processo alocado tem que poder fazer 2 ut de quantum
                count = 0;
            }
            
            insereFila(&alta, p_fita); // Processo volta para a fila de alta prioridade
            removeFila(&fita);

            if(filaVazia(&fita)){
                cont_reg_fita = -1;
            }
            else if(!(filaVazia(&fita))){
                cont_reg_fita = T_FITA;
            }
            
            processos[p_fita].status = 0;
            processos[p_fita].qntdIO--; 
        }

        if (cont_reg_impressora > 0) {
            cont_reg_impressora--;
        } else if (cont_reg_impressora == 0) {
            p_imp = peek(&impressora);
            printf("Processo %d completou IO de impressora e voltou para fila de alta prioridade\n", p_imp);

            if(filaVazia(&alta)){ // se a fila alta estiver vazia, o processo alocado tem que poder fazer 2 ut de quantum
                count = 0;
            }

            insereFila(&alta, p_imp); // Processo volta para a fila de alta prioridade
            removeFila(&impressora);

            if(filaVazia(&impressora)){
                cont_reg_impressora = -1;
            }
            else if(!(filaVazia(&impressora))){
                cont_reg_impressora = T_IMP;
            }

            processos[p_imp].status = 0;
            processos[p_imp].qntdIO--; 
        } 
    }    
       ...
       //se o processo nao saiu pra IO:        
        if((count == quantum && ut != 0) || (p.tempo_esperado <= 0 && p.status != 2)){
            if(f == 0){ // se tem processo na fila de alta prioridade
                printf("Elemento %d retirado da fila de alta\n", peek(&alta));
                removeFila(&alta);
                printFila(&alta);
            }
            else if(f == 1){ // se tem processo na fila de baixa prioridade
                printf("Elemento %d retirado da fila de baixa\n", peek(&baixa));
                removeFila(&baixa);
                printFila(&baixa);
            }
